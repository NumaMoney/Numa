  ...
  // Utilization Settings
    ...
    function getFullUtilizationInterest(
        ...
        if (_utilization < MIN_TARGET_UTIL) {
            ...
            // 18 decimals
            _newFullUtilizationInterest = ((_fullUtilizationInterest * (RATE_HALF_LIFE * 1e36)) / _decayGrowth);
            ...
            uint256 _decayGrowth = (RATE_HALF_LIFE * 1e36) + (_deltaUtilization * _deltaUtilization * _deltaTime);
            // 18 decimals
            _newFullUtilizationInterest = ((_fullUtilizationInterest * _decayGrowth) / (RATE_HALF_LIFE * 1e36));
        } else {
     ...
     */
    function getBorrowRate(uint cash, uint borrows, uint reserves,uint deltaTime,uint _OldfullUtilizationRate) public override view returns (uint256 _newRatePerBlock, uint256 _newFullUtilizationInterest) 
    {

        uint _utilization = utilizationRate(cash, borrows, reserves);

        _newFullUtilizationInterest = getFullUtilizationInterest(deltaTime, _utilization, _OldfullUtilizationRate);

        // _vertexInterest is calculated as the percentage of the delta between min and max interest
        uint256 _vertexInterest = (((_newFullUtilizationInterest - ZERO_UTIL_RATE) * VERTEX_RATE_PERCENT) / RATE_PREC) + ZERO_UTIL_RATE;
