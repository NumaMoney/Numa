    ...
    function getAmountIn(
        ...
        // return caller.getVaultAmountIn(_amount, _closePos);
         if (
            ...
            uint amountIn = getEstimatedInput(cLstToken.underlying(),cNuma.underlying(),_amount);
            ...
            uint amountIn = getEstimatedInput(cNuma.underlying(),cLstToken.underlying(),_amount);
            amountIn = amountIn + (amountIn * slippage)/1 ether;
            return amountIn;
        } else {
            revert("not allowed");
