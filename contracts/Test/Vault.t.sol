...
contract VaultTest is Setup, ExponentialNoError {
    ...
    function checkPrices(uint inputreth,uint inputnuma,uint supply,uint balEthMinusSynthValue) internal view
    {
         // BUY
        ...
        // SELL
        uint rEthAmountNoFee = FullMath.mulDiv(FullMath.mulDiv(
            inputnuma,
           balEthMinusSynthValue,
             (numaSupply)),
            1 ether,
    ...
    function test_GetPriceSimple() public view
    {
        ...
        //
        checkPrices(inputreth,inputnuma,numaSupply,
    ...
    function test_GetPriceSimpleDecay() public
    {
        ...
        // DECAY NOT STARTED
        checkPrices(inputreth,inputnuma,numaSupply - removedSupply,
        (vaultBalance*vault.last_lsttokenvalueWei())/1 ether);
        ...
        checkPrices(inputreth,inputnuma,decayedSupply,
        (vaultBalance*vault.last_lsttokenvalueWei())/1 ether);
    ...
    function test_GetPriceWithMintedSynth() public
    {
        ...
        checkPrices(inputreth,inputnuma,numaSupply,
         (vaultBalance*vault.last_lsttokenvalueWei())/1 ether - synthValueEth);
    ...
    function test_GetPriceWithRebase() public
    {
        ...
        uint numaAmount = vault.lstToNuma(inputreth);
        uint priceEth = vaultManager.numaToEth(inputnuma,IVaultManager.PriceType.BuyPrice);
        ...
        uint priceEthAfter = vaultManager.numaToEth(inputnuma,IVaultManager.PriceType.BuyPrice);
        ...
        uint priceEthAfterExtract = vaultManager.numaToEth(inputnuma,IVaultManager.PriceType.BuyPrice);
   ...
   function test_BuySellRwdExtraction() public
   {
        ...
        assertEq(buyAmount,numaAmount);
        assertEq(numa.balanceOf(userA)-balUserA,numaAmount);
        assertEq(rEth.balanceOf(vaultRwdReceiver)-balRwdAddy,estimateRewards);
   ...
   function test_Fees() public
   {
    ...
    uint feesRwd = (vault.fees()*((1 ether - vaultManager.getBuyFee()) *inputreth)/1 ether)/1000;
    // % sent to fee_address
    assertEq(rEth.balanceOf(vaultFeeReceiver) - balFeeAddress, feesRwd);
    // rest is used for numa backing
    assertEq(rEth.balanceOf(address(vault)) - vaultBalance,inputreth-feesRwd);

    // sell
    balFeeAddress = rEth.balanceOf(vaultFeeReceiver);
    vaultBalance = rEth.balanceOf(address(vault));
    numa.approve(address(vault),inputnuma);
    uint receivedREth = vault.sell(inputnuma,vault.numaToLst(inputnuma),userA);
    ...
    feesRwd = (receivedREth * 1 ether)/vaultManager.getSellFeeOriginal() - receivedREth;
    feesRwd = (feesRwd * vault.fees())/1000;
    assertEq(rEth.balanceOf(vaultFeeReceiver) - balFeeAddress, feesRwd);
    // rest is used for numa backing
    assertEq(vaultBalance - rEth.balanceOf(address(vault)),receivedREth+feesRwd);
