...
contract Setup is ExtendedTest, ConstantsTest{//, IEvents {
    ...
    function _setupVaultAndAssetManager() internal
    {
        ...
        vaultOracle = new VaultOracleSingle(address(rEth), PRICEFEEDRETHETH_ARBI, 402 * 86400, UPTIME_FEED_NULL);
        // vault   
        vault = new NumaVault(address(numa), address(rEth), 1 ether, address(vaultOracle),address(numaMinter));
    ...
    {
        numaOracle = new NumaOracle(USDC_ARBI, INTERVAL_SHORT, INTERVAL_LONG, deployer,address(nuAssetMgr));
  
     
        usdcEthConverter = new USDCToEthConverter(PRICEFEEDUSDCUDC_ARBI,HEART_BEAT_CUSTOM,PRICEFEEDETHUSD_ARBI,HEART_BEAT_CUSTOM,UPTIME_FEED_ARBI);

        moneyPrinter = new NumaPrinter(address(numa), address(numaMinter), NUMA_USDC_POOL_ADDRESS,address(usdcEthConverter), INumaOracle(numaOracle),address(vaultManager));
    ...
    function mintNewPool(address token0, address token1, uint24 fee, uint160 currentPrice) internal virtual returns (address)
    {
        (token0, token1) = token0 < token1 ? (token0, token1) : (token1, token0);
        return nonfungiblePositionManager.createAndInitializePoolIfNecessary(token0, token1, fee, currentPrice);
    }
