...
contract VaultMigrationTest is Setup, ExponentialNoError {
    ...
    function buyVault(uint _amountREth,INumaVault _vault) public returns (uint buyResult,uint sellResult) {
    ...
    function deploy_NumaV2() public
    {
        ...
        numaNominalPrice = vaultManagerOld.numaToToken(1 ether,1 ether,1 ether);
    ...
    function deploy_NumaV2_2() public
    {
        ...
        (nuAssetMgr2,numaMinter2,vaultManager2,vaultOracle2,vault2) = _setupVaultAndAssetManager(
            402 * 86400,
            feereceiver,
            rwdreceiver,
            0,
        ...
        // transfer rETh
        vault.withdrawToken(address(rEth),rEth.balanceOf(address(vault)),address(vault2));
        ...
        uint diff = numaSupplyNew - numaSupplyOld -vaultManager.constantRemovedSupply();

        // keep same period
        uint newPeriod = vaultManager.decayPeriod() - (block.timestamp - vaultManager.startTime());
    ...
    function test_MigrateV1V2_Price() public 
    {
        ...
        uint numaNominalPrice2 = vaultManager.numaToEth(1 ether,IVaultManager.PriceType.NoFeePrice);
        console2.log("numaNominalPrice2",numaNominalPrice2);
        uint numaBuyPrice2 = vaultManager.numaToEth(1 ether,IVaultManager.PriceType.BuyPrice);
        console2.log("numaBuyPrice2",numaBuyPrice2);
        uint numaSellPrice2 = vaultManager.numaToEth(1 ether,IVaultManager.PriceType.SellPrice);
    ...
    function test_MigrateV1V2_buysell() public 
    {
        ...
        (uint buyResult,uint sellResult) = buyVault(rethAmount,INumaVault(address(vaultOld)));
        deploy_NumaV2();
        (uint buyAmount2,uint sellResult2) = buyVault(rethAmount,INumaVault(address(vault)));
        ...
        assertApproxEqAbs(sellResult2,sellResult,0.001 ether,"sell not match");
    ...
    function test_MigrateV2V2() public 
    {
        ...
        (uint buyResult,uint sellResult) = buyVault(rethAmount,INumaVault(address(vault)));

        // ref prices
        numaNominalPrice = vaultManager.numaToEth(1 ether,IVaultManager.PriceType.NoFeePrice);
        numaBuyPrice = vaultManager.numaToEth(1 ether,IVaultManager.PriceType.BuyPrice);
        numaSellPrice = vaultManager.numaToEth(1 ether,IVaultManager.PriceType.SellPrice);
        ...
        // check prices now that numa supply matches
        uint numaNominalPrice2 = vaultManager2.numaToEth(1 ether,IVaultManager.PriceType.NoFeePrice);
        uint numaBuyPrice2 = vaultManager2.numaToEth(1 ether,IVaultManager.PriceType.BuyPrice);
        uint numaSellPrice2 = vaultManager2.numaToEth(1 ether,IVaultManager.PriceType.SellPrice);
        ...
        (numaOracle2,usdcEthConverter2,moneyPrinter2) = _setupPrinter(address(nuAssetMgr2),address(numaMinter2),address(vaultManager2));
       ...
       vault2.setMinLiquidationsPc(250); //25% min
       ...
       (uint buyResult2,uint sellResult2) = buyVault(rethAmount,INumaVault(address(vault2)));
